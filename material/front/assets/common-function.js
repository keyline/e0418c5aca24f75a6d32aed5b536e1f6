        ...
        if ($(this).hasClass("ckeditor")) {
            if (CKEDITOR.instances[$(this).attr('id')].getData() == '') {
                if (msgbox != '') {
                    ...
                    swalAlert('Please enter your ' + $(this).attr('data-check') + ' !!!', 'warning');
                }
                flag = false;
                return false;
            } else {
                ...
                if ($(this).attr('data-check') == 'Phone' || $(this).attr('data-check') == 'Mobile') {
                    if ($.trim($(this).val()).length < 10) {
                        ...
                        if (msgbox != '') {
                            ...
                            swalAlert('Please enter a valid phone number !!!', 'warning');
                        }
                        flag = false;
                        return false;
                    }
...
function checkPassword(pwd, cnfPwd){
	...
	if (!$('#' + pwd).val().match(lowerCaseLetters)) {
		...
		swalAlert('Password must contain one lower case letter, one upper case letter, one number & six(6) characters !!!', 'warning', 10000);
		return false;
	}
