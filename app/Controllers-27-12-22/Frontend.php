...
class Frontend extends BaseController
{
	...
	public function index()
	{
        ...
        $speakerOrderBy[0]          = ['field' => 'rank', 'type' => 'ASC'];
        $data['speakers']           = $this->common_model->find_data('speakers', 'array', ['published' => 1], '', '', '', $speakerOrderBy);
        $orderBy[0]                 = ['field' => 'id', 'type' => 'DESC'];
        $sponsorOrderBy[0]          = ['field' => 'rank', 'type' => 'ASC'];
        $data['imageGallery']       = $this->common_model->find_data('image_gallery', 'array', ['published' => 1, 'is_home_page' => 1], '', '', '', $orderBy, 20);
        $data['videoGallery']       = $this->common_model->find_data('video_gallery', 'array', ['published' => 1, 'is_home_page' => 1], '', '', '', $orderBy);
        $data['sponsors1']          = $this->common_model->find_data('sponsors', 'array', ['published' => 1, 'sponsor_type' => 'HOST STATE'], '', '', '', $sponsorOrderBy);
        $data['sponsors2']          = $this->common_model->find_data('sponsors', 'array', ['published' => 1, 'sponsor_type' => 'GOLD SPONSORS'], '', '', '', $sponsorOrderBy);
        $data['sponsors3']          = $this->common_model->find_data('sponsors', 'array', ['published' => 1, 'sponsor_type' => 'SPONSORS'], '', '', '', $sponsorOrderBy);
        $data['staticPage']         = $this->common_model->find_data('sms_static_content', 'row', ['published' => 1, 'static_id' => 2]);
        $data['event']              = $this->common_model->find_data('sms_events', 'row', ['published' => 1, 'id' => 1]);
